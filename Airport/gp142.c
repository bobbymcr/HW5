...
** 
** RCS $Id: gp142.c,v 1.12 1998/07/16 17:12:38 boren Exp $
**
...
#define GP142_VERSION   "2.2"
#define GP142_DATE      "July 1997"        

#define TIME_PER_STEP 100
...
/* Hmmm... The following pragma is problematic.  
   In summer 95 we had much trouble with PC users reporting errors
   relating to "floating point support not loaded" or the like,
   usually triggered by attempts to printf with a %d format.  Paul
   Schimpf reported the following pragma fixed the problem when
   building the .dll with the Borland compiler, but Casey Anderson
   reports it causes troubles with both Borland Turbo and with MSVC.
   Until someone has time to sort this out, I suggest you leave it
   commented out, unless you start getting similar-sounding errors
   related to floating point conversions.     --- W.L.Ruzzo 10/95
*/
/*
         ...
         LPSTR lpszCmdLine, int nCmdShow)
{
    WNDCLASS wc;
         ...
         if (!hPrevInstance) /* initialize window classes when first run */
                  {
         ...
         hTextWnd = CreateWindow(
                  ...
                  WS_OVERLAPPEDWINDOW,
                  /* 2*x/3 + 1 */0,                   /* position in lower third */
                  2*y/3 + 1,
                  x/*/3*/,                            /* maximum width */
                  y/3,                                /* 1/3 maximum height */
...
/***TODO***/
   ...
   if ((font_info->thefont = XLoadQueryFont(display, 
...
/*       SelectWindow((void *)gpWin);
         ...
         if (PeekMessage(&msg, (HWND)NULL, 0, 0, PM_REMOVE))
                  {
                                /* a message is pending: process it */
    ...
    switch (message)
        {
            ...
            case WM_PAINT:
                ...
                /* Walk through message queue, printing messages */
...
#ifdef WINDOWS
/*  DrawString() prints a line of text in the Text Window */
static void DrawString(char far *s)
{
    ...
    /* we want to use ScrollDC because ScrollWindow automatically appends
    the scrolled region to the window's update region, and sends a
    WM_PAINT to that window.  But DrawString() may very well be
    servicing a WM_PAINT message, and we would be causing another! */
    ...
    /* to know exactly where the line of text should go, we need to know
    what the client window looks like */
    
    GetClientRect(hTextWnd, &rClient);
    
    /* we want to erase the line the text is about to go on.  This way,
    if the new string to print is shorter than the previous, we won't
    have extra characters at the end. */
    ...
    TextOut(hDC,
        rClient.left + 5,               /* leave ~5 pixel left margin */
        rClient.bottom - nScrollHeight, /* new line appears at bottom */
        s,                              /* the string to display (far pointer) */
        ...
        case obj_text:
           ...
           if (item->g.text.ptsize != current_font->point_size) {
              ...
              if (!temp) {
                 ...
                 if ((temp->thefont = XLoadQueryFont(display, 
                                               fontname)) == NULL) {
